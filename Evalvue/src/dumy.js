...
export default function Login({ navigation }) {
  ...
  const _loginApiCalling = async () => {
    ...
    axios.post(apiUrl, postData, { headers })
      .then(async (response) => {
        console.log("LoginResponseStatus99999==>",response.data);
        // if (response.data.ErrorCode == "200") {
          if (response.status == "200"){
          global.props.hideLoader();
          await localStorage.setItemString("isLogin", "1")
          await localStorage.setItemString("authtoken", response.data.authtoken)
          await localStorage.setItemObject("UserData", response.data.getAllActiveModelusers)
          navigation.navigate('DailyexerciseList')
          console.log("LoginUserData==>4444444",response.data.authtoken);
          clearData()
        } else {
